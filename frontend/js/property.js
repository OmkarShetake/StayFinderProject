...
const PropertyPage = {
  ...
  async loadProperty() {
    try {
      ...
      Utils.toast('Failed to load property. Please try again.');
      document.getElementById('detail-title').textContent = 'Property not found';
    }
  },
  ...
  _renderProperty() {
    ...
    document.getElementById('detail-rating').innerHTML  =
        p.avgRating > 0
            ? `★ ${Number(p.avgRating).toFixed(2)} · <span class="underline">${p.totalReviews} reviews</span>`
            : '';
    document.getElementById('detail-location').textContent =
        `${p.city}, ${p.state}, ${p.country}`;
    if (p.host?.superhost) {
      document.getElementById('detail-superhost').classList.remove('hidden');
    }
    ...
    if (imgs[0]) photoMain.innerHTML   = `<img src="${Utils.esc(imgs[0])}" alt="${Utils.esc(p.title)}">`;
    if (imgs[1]) photoThumb1.innerHTML = `<img src="${Utils.esc(imgs[1])}" alt="">`;
    if (imgs[2]) photoThumb2.innerHTML = `<img src="${Utils.esc(imgs[2])}" alt="">`;

    // Host section
    document.getElementById('host-title').textContent =
        `${Utils.propertyTypeLabel(p.propertyType)} hosted by ${p.host?.fullName}`;
    document.getElementById('host-sub').textContent =
        `${p.maxGuests} guests · ${p.bedrooms} bedroom${p.bedrooms > 1 ? 's' : ''} · ` +
        `${p.beds} bed${p.beds > 1 ? 's' : ''} · ${p.bathrooms} bath${p.bathrooms > 1 ? 's' : ''}`;
    document.getElementById('host-avatar').textContent =
        Utils.initials(p.host?.fullName || '');

    if (p.host?.superhost) {
      document.getElementById('host-superhost-badge').classList.remove('hidden');
    ...
    amenGrid.innerHTML = amenities.map(a => `
      <div class="amenity-item">
        <span class="amenity-icon">${Utils.amenityIcon(a)}</span>
        ${Utils.esc(a.replace(/_/g, ' '))}
      </div>`
    ).join('');

    // Booking card
    document.getElementById('bk-price').innerHTML =
        `${Utils.formatCurrency(p.pricePerNight)} <span>night</span>`;
    document.getElementById('bk-rating').textContent =
        p.avgRating > 0
  ...
  async loadAvailability() {
    ...
    try {
      ...
      // Non-critical — calendar will show all dates as available
    ...
    try {
      ...
      if (summary) {
        ...
        if (hdr && summary.overall > 0) {
          hdr.textContent =
              `★ ${Number(summary.overall).toFixed(2)} · ${summary.totalReviews} reviews`;
        }
      }
      ...
      container.innerHTML = reviews.map(r => `
        <div class="review-item">
          <div class="review-header">
            ...
            <div>
              <div class="reviewer-name">${Utils.esc(r.guestName)}</div>
      ...
      if (container) {
        container.innerHTML =
            '<p class="text-muted">Could not load reviews. Please refresh.</p>';
      }
    }
  },
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (isPast || isBooked) {
        ...
        if      (date.getTime() === ci.getTime()) cls += ' rs';
          ...
          date.getTime() === new Date(this.checkIn).getTime()) {
        cls += ' sel';
      }

      const click = (!isPast && !isBooked)
          ? `onclick="PropertyPage.pickDay('${dateStr}')"`
          : '';
  ...
  _updateDateDisplay() {
    document.getElementById('ci-val').textContent =
        Utils.formatDate(this.checkIn);
    document.getElementById('co-val').textContent =
        Utils.formatDate(this.checkOut);
    document.getElementById('ci-val').className =
        'date-cell-val' + (this.checkIn  ? ' set' : '');
    document.getElementById('co-val').className =
        'date-cell-val' + (this.checkOut ? ' set' : '');
  },
  ...
  changeGuests(delta) {
    ...
    document.getElementById('guest-val').textContent   =
  ...
  async _loadPricePreview() {
    ...
    try {
      ...
      document.getElementById('pb-label').textContent    =
          `${Utils.formatCurrency(data.pricePerNight)} × ${nights} night${nights > 1 ? 's' : ''}`;
      ...
      console.error('Price preview failed:', e.message);
      Utils.toast('Could not calculate price. Please try again.');
    }
  },
  ...
  async doReserve() {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = this._rootPath() + 'pages/trips.html';
      }, 1500);
    } catch (e) {
  ...
  _bindEvents() {
    ...
    document.getElementById('photo-grid')?.addEventListener('click', (e) => {
      ...
      let idx = 0;
      thumbs.forEach((t, i) => { if (t.contains(e.target)) idx = i + 1; });
  ...
  _renderLightbox() {
    const imgs  = this._lightboxImages;
